...
#define _DPDK_LOCKLESS_RING_ENQUEUE(elt, in_obj) \
    do { \
        __atomic_store_n(&elt.obj, in_obj, __ATOMIC_SEQ_CST); \
        __atomic_store_n(&elt.state, DPDK_LOCKLESS_RING_STATE_BUSY, __ATOMIC_SEQ_CST); \
    } while (0)

#define _DPDK_LOCKLESS_RING_DEQUEUE(out_obj, elt) \
    do { \
        __atomic_store_n(&out_obj, elt.obj, __ATOMIC_SEQ_CST); \
        __atomic_store_n(&elt.state, DPDK_LOCKLESS_RING_STATE_FREE, __ATOMIC_SEQ_CST); \
    } while (0)
...
try_again:
    do {
        ...
        next_idx = prod_idx + i;
        success = __atomic_compare_exchange(&ring->prod_idx, &prod_idx, &next_idx, true, __ATOMIC_SEQ_CST,
                                            __ATOMIC_SEQ_CST);
...
try_again:
    do {
        prod_idx = __atomic_load_n(&ring->prod_idx, __ATOMIC_SEQ_CST);
        cons_idx = __atomic_load_n(&ring->cons_idx, __ATOMIC_SEQ_CST);
        ...
        for (i = 0; i < n; i++) {
            ...
            if (unlikely(__atomic_load_n(&ring->elts[idx].state, __ATOMIC_SEQ_CST) != DPDK_LOCKLESS_RING_STATE_BUSY)) {
                if (i != 0) {
                    break;
                } else if (cons_idx != __atomic_load_n(&ring->cons_idx, __ATOMIC_SEQ_CST)) {
                    goto try_again;
                } else {
                    return 0;
                }
        ...
        next_idx = cons_idx + i;
        success = __atomic_compare_exchange(&ring->cons_idx, &cons_idx, &next_idx, true, __ATOMIC_SEQ_CST,
                                            __ATOMIC_SEQ_CST);
    } while (unlikely(success == 0));
...
{
    uint32_t prod_idx = __atomic_load_n(&ring->prod_idx, __ATOMIC_SEQ_CST);
    uint32_t cons_idx = __atomic_load_n(&ring->cons_idx, __ATOMIC_SEQ_CST);
...
/*
	...
	*/
    ...
    g_test_ring = dpdk_lockless_ring_alloc("NRT_to_RT_ring_test_cst", EM_LOCKLESS_RING_SIZE);
	if (!g_test_ring) {
