...
try_again:
    do {
        prod_idx = __atomic_load_n(&ring->prod_idx, __ATOMIC_RELEASE);
        cons_idx = __atomic_load_n(&ring->cons_idx, __ATOMIC_RELEASE);
        ...
        for (i = 0; i < n; i++) {
            ...
            if (unlikely(__atomic_load_n(&ring->elts[idx].state, __ATOMIC_RELEASE) != DPDK_LOCKLESS_RING_STATE_BUSY)) {
                if (i != 0) {
                    break;
                } else if (cons_idx != __atomic_load_n(&ring->cons_idx, __ATOMIC_RELEASE)) {
                    goto try_again;
                } else {
                    return 0;
                }
        ...
        next_idx = cons_idx + i;
        success = __atomic_compare_exchange(&ring->cons_idx, &cons_idx, &next_idx, true, __ATOMIC_RELEASE,
                                            __ATOMIC_RELAXED);
    } while (unlikely(success == 0));
